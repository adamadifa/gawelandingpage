...
<head>
    ...
    <title>{{ $settings['brand_name'] ?? 'PresensiGPS' }} - {{ $settings['tagline'] ?? 'Solusi Absensi & Payroll Modern' }}</title>
    <meta name="description" content="Kelola Absensi & Payroll Karyawan Lebih Efisien, Transparan, & Akurat dengan PresensiGPS.">
...
<body class="font-sans text-gray-900 antialiased overflow-x-hidden bg-white">
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="flex items-center justify-between">
                ...
                <a href="/" class="flex items-center gap-3 group text-decoration-none">
                    <div class="w-12 h-12 flex items-center justify-center transition-all duration-500 overflow-hidden group-hover:scale-105">
                        @if(isset($settings['brand_logo']) && $settings['brand_logo'])
                            <img src="{{ asset('storage/' . $settings['brand_logo']) }}" alt="{{ $settings['brand_name'] ?? 'Logo' }}" class="w-full h-full object-contain filter group-hover:brightness-110 transition-all duration-500">
                        @else
                            <div class="w-full h-full bg-brand-600 rounded-[0.9rem] flex items-center justify-center shadow-inner">
                                <span class="text-white font-display font-black text-xl leading-none">{{ substr($settings['brand_name'] ?? 'P', 0, 1) }}</span>
                            </div>
                        @endif
                    </div>
                    <div class="flex flex-col">
                        <span class="font-display text-xl font-black tracking-tight leading-none group-hover:text-brand-700 transition-colors">
                            @if(isset($settings['brand_name']))
                                ...
                                @endphp
                                <span class="text-brand-700">{{ $firstWord }}</span><span class="text-gray-900">{{ $rest }}</span>
                            @else
                                <span class="text-brand-700">Presensi</span><span class="text-gray-900">GPS</span>
                            @endif
                        </span>
                        @if(isset($settings['tagline']))
                            <span class="text-[9px] font-black text-gray-400 uppercase tracking-[0.15em] mt-1">{{ $settings['tagline'] }}</span>
                        @endif
                    </div>
                </a>
                ...
                <div class="hidden md:flex items-center gap-8">
                    <a href="#features" class="text-[15px] font-medium text-gray-600 hover:text-brand-700 transition-colors duration-300">Fitur</a>
                    <a href="#pricing" class="text-[15px] font-medium text-gray-600 hover:text-brand-700 transition-colors duration-300">Harga</a>
                    <a href="#faq" class="text-[15px] font-medium text-gray-600 hover:text-brand-700 transition-colors duration-300">FAQ</a>
                    <a href="#contact" class="text-[15px] font-medium text-gray-600 hover:text-brand-700 transition-colors duration-300">Kontak</a>
                </div>
                ...
                <div class="hidden md:flex items-center gap-4">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="text-xs font-bold text-gray-400 hover:text-rose-500 transition-colors uppercase tracking-widest">Logout</button>
                    </form>
                </div>
                @else
                <div class="hidden md:flex items-center gap-6">
                    <a href="{{ route('login') }}" class="text-[15px] font-bold text-gray-900 hover:text-brand-700 transition-colors">Login</a>
    ...
    {{-- HERO SECTION --}}
    {{-- ========================================= --}}
    <section class="relative min-h-screen flex items-center pt-28 pb-16 lg:pt-0 lg:pb-0 bg-gradient-to-br from-white via-brand-50/40 to-brand-100/30">

        {{-- Decorative Elements --}}

        {{-- Sparkle decorations --}}
        <div class="absolute top-32 left-12 text-brand-300/40 animate-pulse-soft">
            <svg class="w-10 h-10" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0L14.59 8.41L23 11L14.59 13.59L12 22L9.41 13.59L1 11L9.41 8.41L12 0Z"/></svg>
        </div>
        <div class="absolute bottom-40 left-1/2 text-brand-200/30 animate-pulse-soft" style="animation-delay: 1.5s;">
            <svg class="w-6 h-6" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0L14.59 8.41L23 11L14.59 13.59L12 22L9.41 13.59L1 11L9.41 8.41L12 0Z"/></svg>
        </div>
        <div class="absolute top-60 right-20 text-gray-300/50 animate-pulse-soft" style="animation-delay: 0.8s;">
            <svg class="w-8 h-8" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0L14.59 8.41L23 11L14.59 13.59L12 22L9.41 13.59L1 11L9.41 8.41L12 0Z"/></svg>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 w-full">
            <div class="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
                ...
                <div class="relative z-10 animate-fade-in-up">
                    {{-- News Badge --}}
                    <div class="inline-flex items-center gap-2 bg-white/80 border border-brand-200/60 rounded-full pl-1 pr-4 py-1 mb-8 shadow-sm backdrop-blur-sm">
                    ...
                    {{-- Headline --}}
                    <h1 class="font-display text-[42px] lg:text-[56px] leading-[1.08] font-extrabold text-gray-900 mb-6 tracking-tight">
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex -space-x-3">
                            <div class="w-10 h-10 rounded-full bg-brand-200 border-2 border-white flex items-center justify-center text-xs font-bold text-brand-800">A</div>
                            <div class="w-10 h-10 rounded-full bg-brand-200 border-2 border-white flex items-center justify-center text-xs font-bold text-brand-800">B</div>
                            <div class="w-10 h-10 rounded-full bg-teal-200 border-2 border-white flex items-center justify-center text-xs font-bold text-teal-800">C</div>
                        </div>
                        <div>
                ...
                <div class="relative flex justify-center lg:justify-end animate-fade-in-right">
                    ...
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] lg:w-[420px] lg:h-[420px] rounded-full bg-brand-100/30 blur-3xl pointer-events-none"></div>
                    ...
                    @foreach($hero_stats as $index => $stat)
                        ...
                        @endphp
                        <div class="absolute {{ $positionClasses }} z-20 bg-white/90 backdrop-blur-md rounded-2xl px-5 py-4 shadow-xl shadow-black/5 border border-white/50" style="animation-delay: {{ $index * 0.5 }}s;">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl {{ $colorClasses['bg'] }} flex items-center justify-center">
                                    <i class="ti {{ str_starts_with($stat->icon, 'ti-') ? $stat->icon : 'ti-' . $stat->icon }} {{ $colorClasses['text'] }} text-xl"></i>
                                </div>
                                <div>
            ...
            <div class="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
                ...
                <div class="relative animate-fade-in-up">
                    ...
                    <div class="absolute -top-10 -left-10 text-brand-300/40 animate-pulse-soft">
                        <svg class="w-12 h-12" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0L14.59 8.41L23 11L14.59 13.59L12 22L9.41 13.59L1 11L9.41 8.41L12 0Z"/></svg>
                    </div>
                    ...
                    <div class="relative z-10 rounded-[40px] overflow-hidden h-[480px] lg:h-[620px]">
                        <img src="{{ (isset($about->main_image) && str_starts_with($about->main_image, 'images/')) ? asset($about->main_image) : (isset($about->main_image) ? asset('storage/' . $about->main_image) : asset('images/about-person.png')) }}" 
                             alt="HR Manager using PresensiGPS" 
                             class="w-full h-full object-cover object-top">
                    </div>

                    {{-- Floating Analytics Card --}}
                    <div class="absolute -bottom-6 -left-6 lg:-left-10 z-20 w-48 lg:w-64 animate-float shadow-2xl shadow-black/10 rounded-2xl lg:rounded-3xl overflow-hidden">
                    ...
                    <div class="absolute top-10 -left-6 z-20 w-16 h-16 bg-brand-600 rounded-3xl flex items-center justify-center shadow-xl shadow-brand-600/30 animate-float-delayed">
                    ...
                    <ul class="space-y-5 mb-10">
                        @if(isset($about->feature_items) && is_array($about->feature_items))
                            ...
                            <li class="flex items-center gap-4 group">
                                <div class="w-6 h-6 rounded-full bg-brand-500 flex items-center justify-center transition-transform group-hover:scale-110">
                        ...
                        @else
                            <li class="flex items-center gap-4 group">
                                ...
                                <span class="text-gray-700 font-semibold text-lg">Dashboard Analitik Terpusat & Real-time</span>
                            </li>
                        @endif
                    </ul>

                    {{-- CTA Button --}}
                    <a href="{{ $about->cta_url ?? '#pricing' }}" class="inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white font-bold px-8 py-4 rounded-full shadow-lg shadow-brand-600/25 hover:shadow-brand-600/40 transition-all duration-300 hover:translate-x-2">
    ...
    <section id="features" class="py-24 lg:py-32 bg-white overflow-hidden">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="text-center max-w-3xl mx-auto mb-20 animate-fade-in-up">
                <div class="inline-flex items-center gap-2 bg-brand-50 text-brand-700 px-4 py-2 rounded-full mb-6">
                    <span class="text-sm font-bold uppercase tracking-wider">{{ $feature_section->title_badge ?? 'Keunggulan Kami' }}</span>
            ...
            <div class="grid lg:grid-cols-3 gap-12 lg:gap-8 items-center">
                ...
                <div class="space-y-10 lg:space-y-16 order-2 lg:order-1">
                    ...
                    <div class="group flex items-start gap-6 animate-fade-in-up" style="animation-delay: {{ $loop->index * 0.1 }}s">
                        <div class="w-16 h-16 shrink-0 bg-white border border-gray-100 rounded-2xl flex items-center justify-center shadow-sm group-hover:border-brand-200 group-hover:shadow-lg group-hover:shadow-brand-500/5 transition-all duration-300">
                            <i class="ti {{ str_starts_with($feature->icon, 'ti-') ? $feature->icon : 'ti-' . $feature->icon }} text-3xl text-brand-600"></i>
                        </div>
                        <div>
                ...
                <div class="relative flex justify-center order-1 lg:order-2 mb-16 lg:mb-0 animate-fade-in-up">
                    ...
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] lg:w-[500px] lg:h-[500px] rounded-full bg-brand-100/40 blur-[100px] pointer-events-none"></div>
                    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] lg:w-[450px] lg:h-[450px] rounded-full border border-brand-200/30 opacity-50 pointer-events-none"></div>
                ...
                <div class="space-y-10 lg:space-y-16 order-3">
                    ...
                    <div class="group flex items-start gap-6 animate-fade-in-up" style="animation-delay: {{ ($loop->index + 3) * 0.1 }}s">
                        <div class="w-16 h-16 shrink-0 bg-white border border-gray-100 rounded-2xl flex items-center justify-center shadow-sm group-hover:border-brand-200 group-hover:shadow-lg group-hover:shadow-brand-500/5 transition-all duration-300">
                            <i class="ti {{ str_starts_with($feature->icon, 'ti-') ? $feature->icon : 'ti-' . $feature->icon }} text-3xl text-brand-600"></i>
                        </div>
                        <div>
    ...
    <section id="pricing" class="py-24 lg:py-32 bg-gradient-to-b from-gray-50 to-white">
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 items-start">
                ...
                <div class="relative bg-white rounded-3xl p-8 lg:p-10 border-2 transition-all duration-500 hover:-translate-y-2
                    ...
                    <div class="absolute -top-4 left-1/2 -translate-x-1/2">
                        <span class="inline-block bg-brand-600 text-white text-xs font-bold px-5 py-1.5 rounded-full shadow-lg shadow-brand-600/30">Paling Populer</span>
                    </div>
                    @endif
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            <div class="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
                ...
                <div class="animate-fade-in-up">
                    {{-- Badge --}}
                    <div class="inline-flex items-center gap-2 bg-brand-50 text-brand-700 px-4 py-2 rounded-full mb-6">
                        <span class="text-sm font-bold uppercase tracking-wider">{{ $faq_section->title_badge ?? 'FAQs' }}</span>
                    ...
                    <div class="space-y-4">
                        @foreach($faqs as $faq)
                        <div class="faq-item group bg-gray-50/50 border border-gray-100 rounded-2xl overflow-hidden transition-all duration-300 hover:border-brand-200 hover:shadow-xl hover:shadow-brand-500/5">
                            <button class="faq-trigger w-full px-6 py-5 flex items-center justify-between text-left">
                                <span class="font-bold text-gray-900 group-hover:text-brand-600 transition-colors">{{ $faq->question }}</span>
                                <div class="w-8 h-8 rounded-lg bg-white border border-gray-100 flex items-center justify-center transition-transform duration-300 faq-icon-box">
                                    <svg class="w-4 h-4 text-brand-600 transition-transform duration-300 faq-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>
                                </div>
                            </button>
                ...
                <div class="relative animate-fade-in-right h-[400px] sm:h-[500px] lg:h-[650px] lg:-mt-48">
                    ...
                    <div class="absolute top-1/2 right-[-100px] -translate-y-1/2 w-[400px] h-[400px] lg:w-[800px] lg:h-[800px] rounded-full bg-brand-50 lg:bg-brand-100/30 blur-[120px] pointer-events-none"></div>
                    
                    {{-- Background Image (Landscape Dashboard) --}}
                    <div class="absolute top-1/2 right-[-50px] lg:right-[-350px] -translate-y-1/2 w-[350px] sm:w-[500px] lg:w-[1000px] shadow-2xl rounded-3xl overflow-hidden animate-float-delayed border border-white/40 z-10 transition-all duration-500">
                    ...
                    <div class="absolute top-1/2 left-4 sm:left-10 lg:left-[20px] -translate-y-1/2 w-[110px] sm:w-[145px] lg:w-[185px] animate-float z-30 group">
                         <div class="relative drop-shadow-[0_20px_60px_rgba(0,0,0,0.2)] group-hover:scale-105 transition-transform duration-700">
    ...
    <section class="py-20 bg-gradient-to-r from-brand-700 via-brand-600 to-brand-500 relative overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto px-6 lg:px-8 text-center relative z-10">
            <h2 class="font-display text-3xl lg:text-5xl font-bold text-white mb-6">Mulai Kelola Karyawan Anda Sekarang</h2>
            <p class="text-brand-100 text-lg mb-10 max-w-2xl mx-auto">Dapatkan demo gratis 7 hari tanpa komitmen. Rasakan kemudahan mengelola absensi dan payroll dalam satu platform.</p>
            <div class="flex flex-wrap justify-center gap-4">
                <a href="{{ $hero->cta_url ?? '#' }}" class="inline-flex items-center gap-2 bg-white text-brand-700 font-bold px-8 py-4 rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
                    Coba Gratis Sekarang
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </a>
                <a href="https://wa.me/{{ $settings['whatsapp_number'] ?? '' }}" class="inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white font-bold px-8 py-4 rounded-full border border-white/30 transition-all duration-300 hover:-translate-y-1">
                    💬 Chat via WhatsApp
                </a>
            </div>
        </div>
    ...
    <footer id="contact" class="bg-[#F8F9FA] pt-24 pb-12 relative overflow-hidden">
        ...
        <div class="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden">
            <div class="absolute top-[-10%] translate-x-[-30%] left-0 w-[400px] h-[400px] rounded-full bg-brand-100/30 blur-[100px] opacity-60"></div>
            <div class="absolute bottom-0 translate-x-[30%] right-0 w-[500px] h-[500px] rounded-full bg-brand-100/20 blur-[120px] opacity-40"></div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
            <div class="grid lg:grid-cols-12 gap-12 lg:gap-16 items-start pb-20 border-b border-gray-100">
                ...
                <div class="lg:col-span-4 max-w-sm">
                    <a href="/" class="flex items-center gap-3 mb-8 group text-decoration-none">
                        <div class="w-12 h-12 flex items-center justify-center transition-all duration-500 overflow-hidden group-hover:scale-105">
                            ...
                            @else
                                <div class="w-full h-full bg-brand-600 rounded-[1rem] flex items-center justify-center shadow-inner">
                                    <span class="text-white font-display font-black text-xl leading-none">{{ substr($settings['brand_name'] ?? 'P', 0, 1) }}</span>
                                </div>
                            @endif
                        </div>
                        <span class="font-display text-2xl font-black text-gray-900 tracking-tight leading-none group-hover:text-brand-700 transition-colors text-decoration-none">
                            @if(isset($settings['brand_name']))
                                @php
                                    $words = explode(' ', $settings['brand_name']);
                                    $firstWord = $words[0] ?? '';
                ...
                <div class="lg:col-span-2">
                    ...
                    <ul class="space-y-4">
                        <li><a href="#" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Beranda</a></li>
                        <li><a href="#about" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Tentang Kami</a></li>
                        <li><a href="#features" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Fitur Unggulan</a></li>
                        <li><a href="#pricing" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Paket Harga</a></li>
                    </ul>
                </div>
                ...
                <div class="lg:col-span-2">
                    ...
                    <ul class="space-y-4">
                        <li><a href="#faq" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Pusat FAQ</a></li>
                        <li><a href="#" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none">Panduan</a></li>
                        <li><a href="/login" class="text-gray-500 hover:text-brand-600 font-bold text-sm transition-colors uppercase tracking-tight text-decoration-none text-decoration-none">Admin Portal</a></li>
                    </ul>
                </div>

                {{-- Column 4: Contact Card --}}
                <div class="lg:col-span-4">
                    <div class="bg-white p-8 lg:p-10 rounded-3xl shadow-2xl shadow-gray-200 border border-gray-50 relative overflow-hidden group">
                        {{-- Subtle background decoration in card --}}
                        <div class="absolute -top-10 -right-10 w-32 h-32 bg-brand-50 rounded-full blur-2xl opacity-50 transition-transform group-hover:scale-150 duration-700"></div>
                        
                        <div class="relative z-10">
                            <span class="text-brand-600 font-bold text-sm uppercase tracking-widest mb-4 inline-block">Hubungi Kami</span>
                            ...
                            <p class="text-gray-500 text-sm leading-relaxed mb-10 font-medium">
                                Berikan tim HR Anda alat yang tepat untuk mengelola aset terpenting perusahaan: Karyawan.
                            </p>
                            
                            <div class="space-y-6">
                                <div class="flex items-start gap-4">
                                    <div class="w-10 h-10 shrink-0 bg-brand-50 rounded-xl flex items-center justify-center text-brand-600 shadow-sm">
                                        <i class="ti ti-mail text-xl"></i>
                                    </div>
                                    <div class="leading-tight pt-1">
                                        <p class="text-[11px] text-gray-400 font-bold mb-0.5 uppercase tracking-wider">Email Kami</p>
                                        <p class="text-gray-800 font-bold text-sm">{{ $settings['email_contact'] ?? 'hello@' . strtolower(str_replace(' ', '', $settings['brand_name'] ?? 'presensigps')) . '.com' }}</p>
                                    </div>
                                </div>
                                <div class="flex items-start gap-4">
                                    <div class="w-10 h-10 shrink-0 bg-brand-50 rounded-xl flex items-center justify-center text-brand-600 shadow-sm">
                                    ...
                                    <div class="leading-tight pt-1">
                                        <p class="text-[11px] text-gray-400 font-bold mb-0.5 uppercase tracking-wider">Hubungi Kami</p>
                                        <p class="text-gray-800 font-bold text-sm">{{ $settings['whatsapp_number'] ?? '+62 812 3456 7890' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            {{-- Bottom Bar --}}
            <div class="pt-12 mt-20 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-6 text-gray-400 text-xs font-bold uppercase tracking-widest">
                <div class="flex items-center gap-2">
                    <p class="font-medium">&copy; {{ date('Y') }} {{ $settings['brand_name'] ?? 'PresensiGPS' }}. All rights reserved.</p>
                </div>
                <div class="flex items-center gap-8">
                    ...
                    <a href="#" class="hover:text-brand-600 font-medium transition-colors uppercase">Powered by Biznet GIO</a>
                </div>
            </div>
        </div>
    </footer>
            ...
            button.addEventListener('click', () => {
                ...
                document.querySelectorAll('.faq-item').forEach(other => {
                    if (other !== item) {
                        ...
                        if (other.querySelector('.faq-icon i')) {
                            other.querySelector('.faq-icon i').style.transform = 'rotate(0deg)';
                        }
