...
@section('title', 'Features — Edit Item')
...
<div class="w-full space-y-8 pb-20 text-inter">
    ...
    <div class="flex items-center justify-between mb-8 pb-6 border-b border-gray-100">
        <div>
            <nav class="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                ...
                <i class="ti ti-chevron-right text-[8px]"></i>
                <span class="text-indigo-600">Edit Fitur</span>
            </nav>
            <h1 class="text-3xl font-display font-black text-gray-900 leading-tight">Perbarui Item Fitur</h1>
            <p class="text-sm text-gray-500 font-medium font-poppins">Sesuaikan poin keunggulan aplikasi yang ditampilkan di landing page.</p>
        </div>
        <div class="flex items-center gap-3">
             ...
             <button type="submit" form="feature-item-edit-form" class="inline-flex items-center gap-2 px-7 py-3.5 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 active:scale-95 transition-all font-poppins">
    ...
    <form id="feature-item-edit-form" action="{{ route('admin.features.update', $feature) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start font-poppins">
        @csrf
        @method('PUT')
        ...
        <div class="lg:col-span-7 space-y-8">
            {{-- Primary Info Card --}}
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-xl shadow-black/[0.02] p-8 lg:p-10 space-y-8 text-poppins">
                <div class="flex items-center gap-4 border-b border-gray-50 pb-6 mb-8 text-poppins">
                ...
                <div class="space-y-6">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title" value="{{ old('title', $feature->title) }}" required placeholder="Contoh: Absensi Geofencing Pintar"
                    ...
                    <div class="space-y-3">
                        ...
                        <textarea name="description" rows="6" required placeholder="Jelaskan secara singkat kegunaan fitur ini bagi pengguna..."
                            class="w-full bg-gray-50/50 border-gray-200 rounded-[1.25rem] py-4 px-6 text-gray-900 font-medium focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-600 transition-all placeholder:text-gray-300 leading-relaxed">{{ old('description', $feature->description) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-6">
                    <div class="space-y-4">
                        ...
                        <div class="relative group">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center transition-all group-focus-within:bg-indigo-600 group-focus-within:text-white" id="icon-preview-box">
                                <i class="ti {{ old('icon', $feature->icon) ?: 'ti-star' }} text-xl" id="icon-preview-el"></i>
                            </span>
                            <input type="text" name="icon" id="icon-input" value="{{ old('icon', $feature->icon) }}" required
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', $feature->sort_order) }}" required
            ...
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-xl shadow-black/[0.02] p-8 space-y-8">
                <div class="flex items-center gap-4 justify-between">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $feature->is_active ? 'checked' : '' }}>
