    ...
    <div class="flex items-center justify-between mb-8 pb-6 border-b border-gray-100">
        <div>
            <nav class="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">
                ...
                <i class="ti ti-chevron-right text-[8px]"></i>
                <span class="text-indigo-600 font-bold">Edit Section</span>
            </nav>
            <h1 class="text-3xl font-display font-black text-gray-900 leading-tight">Perbarui Section FAQ</h1>
            <p class="text-sm text-gray-500 font-medium">Sesuaikan detail headline dan visual mockup untuk mengoptimalkan tampilan FAQ.</p>
        </div>
        <div class="flex items-center gap-3">
             ...
             <button type="submit" form="faq-section-edit-form" class="inline-flex items-center gap-2 px-7 py-3.5 bg-indigo-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-indigo-600/20 hover:bg-indigo-700 active:scale-95 transition-all">
            ...
            <div class="bg-white rounded-[2rem] border border-gray-100 shadow-xl shadow-black/[0.02] p-8 lg:p-10 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title_badge" value="{{ old('title_badge', $faq_section->title_badge) }}" required placeholder="Contoh: FAQs"
                    ...
                    <div class="space-y-3">
                        ...
                        <input type="text" name="title_badge_icon" value="{{ old('title_badge_icon', $faq_section->title_badge_icon) }}" required placeholder="Emoji atau Icon"
                ...
                <div class="space-y-3">
                    ...
                    <textarea name="description" rows="5" required placeholder="Tuliskan paragraf pengantar untuk daftar pertanyaan..."
                        class="w-full bg-gray-50/50 border-gray-200 rounded-[1.25rem] py-4 px-6 text-gray-900 font-medium focus:bg-white focus:ring-4 focus:ring-indigo-100 focus:border-indigo-600 transition-all placeholder:text-gray-300 leading-relaxed">{{ old('description', $faq_section->description) }}</textarea>
                </div>
            </div>
        </div>
                        ...
                        <div class="w-full aspect-video bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-4 relative group overflow-hidden hover:border-indigo-400 transition-all">
                            @if($faq_section->primary_image)
                                <img src="{{ str_starts_with($faq_section->primary_image, 'images/') ? asset($faq_section->primary_image) : asset('storage/' . $faq_section->primary_image) }}" 
                                     class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" id="preview-primary">
                            @else
                            ...
                            @endif
                            <input type="file" name="primary_image" class="absolute inset-0 opacity-0 cursor-pointer z-10" onchange="previewImage(this, 'preview-primary', 'icon-primary', 'text-primary')">
                        </div>
                    </div>
                        ...
                        <div class="w-full aspect-square bg-gray-50 rounded-[2rem] border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-4 relative group overflow-hidden hover:border-indigo-400 transition-all">
                            @if($faq_section->secondary_image)
                                <img src="{{ str_starts_with($faq_section->secondary_image, 'images/') ? asset($faq_section->secondary_image) : asset('storage/' . $faq_section->secondary_image) }}" 
                                     class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" id="preview-secondary">
                            @else
                            ...
                            @endif
                            <input type="file" name="secondary_image" class="absolute inset-0 opacity-0 cursor-pointer z-10" onchange="previewImage(this, 'preview-secondary', 'icon-secondary', 'text-secondary')">
                        </div>
                    </div>
                </div>
            </div>
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                if (!preview) {
                    ...
                    preview.className = "absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500";
